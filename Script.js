...
// ===== FUNGSI TAMPILAN HASIL =====
    ...
    sortedStudents.forEach((student, index) => {
        ...
        if (index > 0 && sortedStudents[index - 1].score === student.score) {
            ...
            // Peringkat baru
            if (ascending) {
                // Peringkat 1 untuk nilai terendah, peringkat terbesar untuk nilai tertinggi
                currentRank = sortedStudents.length - index;
            } else {
                // Peringkat 1 untuk nilai tertinggi
                currentRank = index + 1;
            }
            let rankClass = '';
...
// ===== FUNGSI PENCARIAN SISWA =====
    ...
    if (foundIndex !== -1) {
        ...
        let rank;
        if (ascending) {
            // Peringkat 1 untuk nilai tertinggi (index terakhir)
            rank = sortedStudents.length - foundIndex;
        } else {
            // Peringkat 1 untuk nilai tertinggi (index pertama)
            rank = foundIndex + 1;
        }
        // Penanganan peringkat sama (nilai sama)
        for (let i = foundIndex - 1; i >= 0; i--) {
            if (sortedStudents[i].score === student.score) {
                if (ascending) {
                    rank = sortedStudents.length - i;
                } else {
                    rank = i + 1;
                }
            } else {
